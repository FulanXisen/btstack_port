        ...
        case HCI_EVENT_AVRCP_META:
          switch (hci_event_avrcp_meta_get_subevent_code(packet)) {
            case AVRCP_SUBEVENT_COVER_ART_CONNECTION_ESTABLISHED:
              status =
                  avrcp_subevent_cover_art_connection_established_get_status(
                      packet);
              cid =
                  avrcp_subevent_cover_art_connection_established_get_cover_art_cid(
                      packet);
              if (status == ERROR_CODE_SUCCESS) {
                printf(
                    "Cover Art       : connection established, cover art cid "
                    "0x%02x\n",
                    cid);
...
#ifdef HAVE_POSIX_FILE_IO
                printf(
                    "Cover Art       : download of '%s complete, size %u "
                    "bytes'\n",
            ...
            case AVRCP_SUBEVENT_COVER_ART_CONNECTION_RELEASED:
              ...
              set_a2dp_sink_cover_art_cid(0);
              printf(
                  "Cover Art       : connection released 0x%02x\n",
...
uint8_t cover_art_connect(void) {
  ...
  status = avrcp_cover_art_client_connect(
      get_a2dp_sink_cover_art_client(), cover_art_packet_handler,
