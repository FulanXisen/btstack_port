...
void avrcp_packet_handler(uint8_t packet_type, uint16_t channel,
  ...
  switch (packet[2]) {
    case AVRCP_SUBEVENT_CONNECTION_ESTABLISHED: {
      ...
      avrcp_subevent_connection_established_get_bd_addr(packet,
                                                        *get_device_addr());
#endif
  ...
  switch (packet[2]) {
    ...
    case AVRCP_SUBEVENT_GET_CAPABILITY_EVENT_ID_DONE:
      ...
      for (event_id = (uint8_t)AVRCP_NOTIFICATION_EVENT_FIRST_INDEX;
           event_id < (uint8_t)AVRCP_NOTIFICATION_EVENT_LAST_INDEX;
           event_id++) {
        printf("   - [%s] %s\n",
               (avrcp_connection->notifications_supported_by_target &
                (1 << event_id)) != 0
      ...
      // image handles become invalid on player change, registe for
      // notifications
      avrcp_controller_enable_notification(
          avrcp_connection->avrcp_cid, AVRCP_NOTIFICATION_EVENT_UIDS_CHANGED);
    ...
    case AVRCP_SUBEVENT_NOTIFICATION_EVENT_UIDS_CHANGED:
      if (get_a2dp_sink_cover_art_client_connected()) {
        printf(
            "AVRCP Controller: UIDs changed -> disconnect cover art client\n");
        avrcp_cover_art_client_disconnect(get_a2dp_sink_cover_art_cid());
      }
      break;

    case AVRCP_SUBEVENT_NOW_PLAYING_COVER_ART_INFO:
      if (avrcp_subevent_now_playing_cover_art_info_get_value_len(packet) ==
          7) {
